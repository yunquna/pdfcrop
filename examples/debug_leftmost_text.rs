//! Find the leftmost text operation on a page
//!
//! Usage: cargo run --example debug_leftmost_text <pdf_file> <page_num>
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Finding leftmost text on page {}:", page_num + 1);
    println!("==========================================\n");
    ...
    let mut text_positions: Vec<(usize, f64, f64, String)> = Vec::new();
    ...
    for (i, operation) in content.operations.iter().enumerate() {
        match operation.operator.as_ref() {
            ...
            "Tj" | "TJ" | "'" | "\"" => {
                ...
                text_positions.push((i, text_matrix.e, text_matrix.f, text_preview));
            }
            _ => {}
        }
    }
    ...
    // Sort by x position
    text_positions.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap());

    println!("Leftmost 20 text operations:");
    println!("----------------------------");
    for (i, (op_idx, x, y, preview)) in text_positions.iter().take(20).enumerate() {
        println!("{:2}. Op {:4}: x={:8.2}, y={:8.2}, text: {:?}", i + 1, op_idx, x, y, preview);
    }

    if let Some((_, min_x, _, _)) = text_positions.first() {
